...
class StorageProvider implements IStorageProvider {
  public async save(file: string, output?: string): Promise<string> {
    await fs.copy(path.resolve(StorageConfig.local.tempFolder, file), path.resolve(StorageConfig.local.uploadFolder, output || file));
    return `${output || file}`;
  }
